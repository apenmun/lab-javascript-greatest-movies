...
function howManyMovies(moviesArray) {
    const dramaSpielberg = moviesArray.filter((dramaMov) => dramaMov.director === 'Steven Spielberg' && dramaMov.genre.includes('Drama'));
    return dramaSpielberg.length;
}

// Iteration 3: All scores average - Get the average of all scores with 2 decimals
function scoresAverage(moviesArray) {
    const movieAverage = moviesArray.reduce ((a, b) => { 
        if (b.score !== '' && !b.score){
            return a + b.score;
        }
    },0);
    return movieAverage/moviesArray.length;
}
